...
function PANEL:SetEnabled(b)
    ...
    -- We're probably changing the width of something in our parent by appearing or hiding, so tell them to re-do their layout.
...
function PANEL:SetScroll(scrll)
    ...
    self:InvalidateLayout()
    -- If our parent has a OnVScroll function use that, if not then invalidate layout (which can be pretty slow)
...
function PANEL:OnCursorMoved(x, y)
    ...
    local x, _ = self:ScreenToLocal(0, gui.MouseY())
    -- Uck.
    x = x - self.btnUp:GetTall()
    x = x - self.HoldPos
...
function PANEL:Grip()
    ...
    self.Dragging = true
    local x, _ = self.btnGrip:ScreenToLocal(0, gui.MouseX())
    self.HoldPos = x
...
vgui.Register("chicagoRP_HorizontalScrollBar", PANEL, "Panel")